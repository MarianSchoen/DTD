...
```{r, fig.width = 7, fig.align="center"}
  ...
  print(ggplot_convergence(fista.output = catch$best.model, 
                         main = "DTD Vignette"))
```

Here, we trained the $g$-vector for `r maxit` iterations, which decreased the loss function from `r format(max(catch$best.model$Convergence), digits = 3)`  to `r format(min(catch$best.model$Convergence), digits = 3)`.
...
- tweak.vec: numeric vector, with which the minimization starts. 
...
```{r, fig.width=7}
  test.estimations <- est_cs(X = X.matrix, 
                             Y = test.data$mixtures, 
                             gamma.vec = catch$best.model$Tweak
                            )
...
```{r}
    print(ggplot_ghistogram(fista.output = catch$best.model))
```
...
```{r, fig.show="hold", fig.width=3}
  singlePic <- ggplot_gpath(fista.output = catch$best.model, 
                            number.pics = 1, 
                            main="All genes",
...
```{r}
  singlePic.subset <- ggplot_gpath(fista.output = catch$best.model,
                                   number.pics = 1, 
                                   main = "Gene Subset", 
                                   G.TRANSFORM.FUN = log2, 
                                   y.lab = "log2(g)",
                                   subset = c("gene66", "gene121"))
...
```{r, fig.width=5}
  multPic <- ggplot_gpath(fista.output = catch$best.model, 
                          number.pics = 2,  
...
```{r}
  # set.seed(2018)
  # cv.object <- DTD_cv_lambda(tweak.start = start.tweak, 
  #                            nfolds = 5, 
  #                            lambda.length = 10, 
  #                            cv.verbose = TRUE, 
  ...
  # str(cv.object)
```
The return object from the `DTD_cv_lambda` function is a list with 2 entries. The `cv.obj` entry holds a data.frame with information about the cross validation over the lambda.sequence. Pass this data frame to the `plot.cv` function for visualization. The second entry `best.model` of the `cv.obj` is a trained model which uses the lambda with minimal loss function during the cross validation. Notice that this model is trained on the complete trainings data set. This model is the output of a `descent_generalized_fista` call.
