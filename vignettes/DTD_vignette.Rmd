...
The package DTD learns $g$ by minimizing $L$ on a training set of arteficicial bulk profiles $Y$ simmulated by mixing
known reference profiles with known fractions $C$. 
Here $C_{j,.}$ are the true compostions wheras $\widehat{C_{j,.}}(g)$ are the estimated compositions according to equation (2).
...
In this vingnette we will demonstrate its use on simmulated data. To this end we have generated a matrix of random data reflecting single cell measurements.
In fact the DTD package contains the `generate_random_data` function to simmulate such data.
It has 3 arguments:

- n.types: integer, number of simmulated cell-types. 
- n.samples.per.type: integer, number of simmulated single cell profiles per cell-type.
...
For the simmulated data there was no such transformation.
...
Next, we build the reference matrix $X$. $X$ holds data from some but not all input profiles. The profiles used to buld $X$ can either be selected manually, or chosen randomly. Within the DTD package we provide the `sample_random_X` function, which generates a reference matrix by randomly selecting samples of the same type, and then averaging them to a reference profile for this type. 
...
Here we build $X$ using only the first `r length(include.in.X)` cell types and `r round((100*perc.of.all.cells), digits = 2)` of the corresponding single cell profiles.
```{r echo=FALSE, results = "asis"}
cat("```\n")
...
A column of Y is a simmulated bulk tissue profile, where the coresponding row in $C$ annotes the cellular composition of this tissue. 
From the single cell profiles in the training set (that were not used to build $X$) we randommly sample profiles and average them to generate  
...
* train.data.list: list of training profiles, and compsositions. 
