...
The `mix_samples_with_jitter` function needs additionally a `prob.each` parameter. In the `mix_samples` function the average quantity for each cell type is its occurence in the `exp.data`. In the `mix_samples_with_jitter` function, the average expected quantity for each cell type is set via `prob.each`. `prob.each` must be a numeric vector, with the same length as `included.in.X`. 
