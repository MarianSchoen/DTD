...
All functions passed to `descent_generalized_fista` must not take more than input parameter. However, our loss function depends on 4 arguments which are the matrices C, X and Y, the tweak vector. The  matrices C, X and Y do not change during the optimization, only the tweak vector changes during optimization. In order to have a function that only depends on the tweak argument, we advise to use wrapper function, e.g.: 
```{r}
   # wrapper for gradient:
   DTD.grad.wrapper <- function(tweak){
      X <- X.matrix
