...
```
The output of the `train_correlation_model` algorithm is a list. The entry 'cv.obj' holds a data.frame with information about the cross validation. The entry 'best.model' contains a model trained on the full trainings data set with the best lambda of the cross validation. This model contains the following entries: In the entry `Tweak` it returns the `tweak.vec` after the last iterations (which is that $g$ vector which minimizes the loss function best), and a convergence vector. Within the convergence vector there are the evaluations of the `EVAL.FUN` in every step. Depending on the `save.all.tweaks` argument, there is a third entry named `History`. In this matrix the $g$ vector of every iteration can be found. Additionally, in the 'model' object there is a entry named 'reference.X' and 'pics'. 'reference.X' holds the used reference matrix X, in 'pics' there are several ggplot figures to assess the quality of the model. 

## Visualize results

Each of the following visualizations is called automatically after training a model with the `train_correlation_model` function. They are stored as a list of ggplot object, and can be found in the `pics` entry of the model.   
...
As input it takes the DTD model and a test set. 
...
In order to assess the effect of each feature in the deconvolution process we introduce the concept of 'explained correlation': 
Given a trained model, we can iteratively manipulate the model by excluding a feature. This means, its entry in the g-vector is set to zero. Then we deconvolute the test set with the manipulated model, and observe a decrease in correlation. The higher the decrease is, the more important the gene is in the deconvolution process. In the following figure, the y-axis holds the percentage of explained correlation per feature.

```{r, fig.width = 7, fig.align="center"}
# print via model$pics list:
print(model$pics$explained.correlation)
# or via function call:
print(
  ggplot_explained_correlation(
    DTD.model = model,
    X.matrix = X.matrix,
    test.data = test.data,
    estimate.c.type = "direct",
...
- n.folds: integer, how many buckets in the cross validation 
