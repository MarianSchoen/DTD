...
```
# What is DTD?
The gene expression profile of a tissue averages the expression profiles of all cells in this tissue. Digital tissue deconvolution (DTD) addresses the following inverse problem: Given the expression profile y of a tissue, what is the cellular composition c of that tissue? 
@Goertler2018


In our Digital Tissue Deconvolution (DTD) package we provide functions and routines to deconvolute a bulk sample. 
The underlying idea of our DTD approach is to use loss-function learning. Based on a training sample we use a modified FISTA implementation to minimize 
\[ 
L(g) = 1 - \sum_{celltypes} cor(C_{true}, C_{estimated} (g)) 
\]
After training a g-vector we can use it on knew data set. 
...
```{r}
  ...
  str(catch)
```
The output of the `descent_generalized_fista` algorithm is a list. It returns the `tweak_vec` after the last iterations, and a convergence vector. Within the convergence vector there are stored the evaluations of the `EVAL.FUN` in every step. 
## Visualize results
