...
#'
#' @param cross.val.frame data frame, as return by the DTD_cv_lambda function
...
#' @export
ggplot_cv <- function(cross.val.frame,
                      main = "",
  ...
  if(!is.data.frame(cross.val.frame) ||
     ...
     !is.numeric(LAMBDA.TRANS.FUN(2))
      ...
      stop("In plot.cv: passed \"cross.val.frame\" frame does not fit or LAMBDA.TRANS.FUN does not return numeric.")
    }
