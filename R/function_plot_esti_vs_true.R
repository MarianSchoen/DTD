...
ggplot_true_vs_esti <- function(estimated.c,
  ...
  if(norm.mixturewise){
    estimated.c <- apply(estimated.c, 2, function(x){return(x/sum(x))})
    true.c <- apply(true.c, 2, function(x){return(x/sum(x))})
  }
  # Norm every type to range from 0 to 1
  if(norm.typewise){
    estimated.c <- t(apply(estimated.c, 1, function(x){return((x - min(x))/(max(x) - min(x)))}))
    true.c <- t(apply(true.c, 1, function(x){return((x - min(x))/(max(x) - min(x)))}))
  }
