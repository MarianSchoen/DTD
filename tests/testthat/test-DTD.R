...
test_that("normalize_to_count ", {
  expect_error(normalize_to_count(exp.data = random.data, count = "iwas"),
  ...
  expect_error(normalize_to_count(exp.data = matrix(c(1:8, NA), nrow = 3)),
               "In normalize_to_count: exp.data includes NA")
})
{
...
test_that("sample_random_X", {
  ...
  expect_error(
    ...
    "in sample_random_X: 'names(pheno)' do not fit 'colnames(exp.data)'. For every entry of 'colnames(exp.data)' there has to be an entry in 'pheno'",
    fixed = TRUE
  )
  expect_error(
    sample_random_X(
...
test_that("mix_samples ", {
  expect_error(
    mix_samples(exp.data = train.mat,
...
test_that("evaluate_cor ", {
  ...
  expect_error(
    evaluate_cor(X.matrix = X.matrix,
                 new.data = training.data$mixtures,
                 true.compositions  = training.data$quantities,
  ...
  expect_error(
    ...
    "In evaluate_cor: 'estimate.c.type' does not match 'non_negative' or 'direct."
  )

  expect_equal(
    evaluate_cor(X.matrix = X.matrix[sample(1:nrow(X.matrix)), sample(1:ncol(X.matrix))],
                 new.data = training.data$mixtures[sample(1:nrow(training.data$mixtures)),
                                                   sample(1:ncol(training.data$mixtures))],
                 true.compositions  = training.data$quantities[sample(1:nrow(training.data$quantities)),
                                                               sample(1:ncol(training.data$quantities))],
                 DTD.model = sample(start.tweak),
                 estimate.c.type  = "direct")/ncol(X.matrix),
    evaluate_cor(X.matrix = X.matrix[sample(1:nrow(X.matrix)), sample(1:ncol(X.matrix))],
                 new.data = training.data$mixtures[sample(1:nrow(training.data$mixtures)),
                                                   sample(1:ncol(training.data$mixtures))],
                 true.compositions  = training.data$quantities[sample(1:nrow(training.data$quantities)),
...
context("Test DTD_cv_lambda ")
test_that("DTD_cv_lambda ", {
  expect_error(
    DTD_cv_lambda(lambda.seq = NULL,
                  tweak.start = "1",
                  n.folds = 10,
                  lambda.length = 10,
                  train.data.list = training.data,
...
#   }
# )
context("Test soft_thresholding ")
test_that("soft_thresholding ",
          {expect_error(
            soft_thresholding(x = start.tweak,
                              lambda = c(1,2,3)),
            "In soft_thresholding: length of lambda must either be 1 or same as x."
          )
          }
)
...
test_that("ggplot_true_vs_esti",{
          expect_error(
            ggplot_true_vs_esti(DTD.model = 1:100,
                                X.matrix = NULL,
                                test.data = test.data,
                                estimate.c.type = "direct"),
          ...
          expect_message(
            ggplot_true_vs_esti(DTD.model = catch,
                                X.matrix = -1:10,
                                test.data = test.data,
                                estimate.c.type = "direct"),
